...
@Component({
  ...
  imports: [NgTemplateOutlet, MatIconButton, MatIcon, MatMiniFabButton],
})
export class MatCarouselComponent implements AfterContentInit, AfterViewInit, MatCarousel, OnDestroy {
  ...
  public get currentSlide(): MatCarouselSlideComponent | null {
    if (this.listKeyManager) {
      return this.listKeyManager.activeItem || null;
    }
  ...
  public ngAfterViewInit(): void {
    ...
    this.slides$.pipe(takeUntil(this.destroy$)).subscribe((value) => {
  ...
  private getOffset(): number {
    const offset = (this.listKeyManager?.activeItemIndex || 0) * this.getWidth();
    const sign = this.orientation === 'rtl' ? 1 : -1;
    return sign * offset;
  }
