...
@Injectable()
export class ShareContentStrategy implements ManagementActionStrategy {
  execute(contentType: LearnContentManagementType, item: LearnContentListItem) {
    if (contentType !== 'courses') {
      console.warn(`It is not possible to share ${contentType}.`);
      return of(ContentManagementListActions.executeActionNoopResult());
    }

    return of(
      MissionTransferActions.openDialog({
        data: { mission: item, transferType: MissionTransferType.SHARE },
      }),
    );
  }
}
