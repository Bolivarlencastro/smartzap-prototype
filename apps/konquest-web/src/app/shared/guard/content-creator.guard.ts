...
export class ContentCreatorGuard {
  constructor(
  ...
  private checkUserPermissions(): boolean {
    const isContentCreator = this._userProfileService.isContentCreator();
    const isInstructor = this._userProfileService.hasRoles(['instructor']);
    const canAccess = isContentCreator || isInstructor;
