...
export class UserDetailsService {
  ...
  private _userEnrollmentList = new BehaviorSubject<LazyResponse<EnrollmentListResponse>>({
  ...
  readonly userEnrollmentList$ = this._userEnrollmentList.asObservable();
  ...
  fetchUserEnrollments(userId: string, filters: AnalyticsApiUserEnrollmentsFilter): void {
    this._userEnrollmentList.next({ ...this._userEnrollmentList.getValue(), isLoading: true });

    this._learnAnalyticsService
      .fetchUserEnrollments(userId, filters)
      .pipe(
        tap((response) => {
          response.data?.forEach((enrollment) => {
            const label = this.translateService.translate(
              `${CATEGORIES}.${enrollment._source.category_name_translated}`,
            );
            enrollment._source.category_name_translated = label.startsWith(CATEGORIES)
              ? enrollment._source.category_name_translated
              : label;
          });
        }),
      )
      .subscribe((response) => this._userEnrollmentList.next({ response, isLoading: false }));
  }
}
