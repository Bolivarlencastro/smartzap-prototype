...
it('Should found trail at global search', () => {
  ...
  GlobalSearchElements.buttonTrailOnGlobalSearch().should('be.visible').click();
  cy.wait('@loadTrailSearch');
  cy.GlobalSearch(createdTrail.name);

  cy.intercept(`**/learning-trails/${createdTrail.id}`).as('loadTrail');
  GlobalSearchElements.listItems().should('contain', createdTrail.name);
  GlobalSearchElements.buttonOpenItem(createdTrail.name).scrollIntoView().click({ force: true });
