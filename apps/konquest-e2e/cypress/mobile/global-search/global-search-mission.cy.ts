...
it('Should found mission at global search', () => {
  ...
  GlobalSearchElements.buttonMissionOnGlobalSearch().scrollIntoView().click({ force: true });
  cy.wait('@loadMissionSearch');
  cy.GlobalSearch(createdMission.name);

  cy.intercept(`**/missions/${createdMission.id}`).as('loadMission');
  GlobalSearchElements.mobileItemName().should('contain', createdMission.name).click();
