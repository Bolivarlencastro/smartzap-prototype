...
it('Should found channel at global search', () => {
  ...
  GlobalSearchElements.buttonChannelOnGlobalSearch().should('be.visible').scrollIntoView().click({ force: true });
  cy.wait('@loadChannelSearch');
  cy.GlobalSearch(createdChannel.name);

  cy.intercept(`**/channels/${createdChannel.id}`).as('loadChannel');
  GlobalSearchElements.mobileItemName().should('contain', createdChannel.name).click();
